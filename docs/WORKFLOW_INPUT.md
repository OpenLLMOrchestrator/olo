...
- **Purpose**: **olo-worker-input** defines the model and (de)serialization. The shape above is implemented as `WorkflowInput`, `InputItem`, `Context`, `Routing`, `Metadata`, `Storage` in `com.olo.input.model`. **Storage** uses `@JsonInclude(NON_NULL)` so null cache/file are omitted (e.g. `{"mode":"LOCAL"}` only).
...
- **WorkflowInputSerializer.build(...)** creates a `WorkflowInput` (version 2.0) with: **userQuery** input of **type STRING**, **storage** `{ "mode": "LOCAL" }`, and **value** = plain user message (no JSON wrapper). **Context** includes tenantId, sessionId, **runId**, and **callbackBaseUrl** so the worker can call back to the backend. **Routing** includes pipeline (task queue), transactionType, and transactionId (e.g. runId).
